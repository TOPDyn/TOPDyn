...
def _set_gmsh_options():
    ...
    #gmsh.option.setNumber('Geometry.Quads', 1)
...
def _get_matrices():
    #gmsh.model.mesh.setRecombine(2, gmsh.model.getEntities(2))
    ...
    if (frequencies[:, 1] > 1).any():
        print("TEM BOSTA AQUI MARQUINHO")
...
def _get_connectivity_matrix(total_el, connectivities, nelx, nely):
    ...
    # for i, (index, connect) in enumerate(zip(indexes, split_sequence(connectivities, 4))):
    #     first_node  = map_nodes[connect[0]]
    #     second_node = map_nodes[connect[1]]
    #     third_node  = map_nodes[connect[2]]
    #     last_node   = map_nodes[connect[3]]
    #     connectivity_matrix[i,:] = [map_elements[index], first_node, second_node, third_node, last_node]
