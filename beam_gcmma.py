...
def main(mesh_file, nelx, nely, lx, ly, func_name, force_matrix, restri_matrix=None, freq1=180, constr_func=["area"], constr_values=[50], n1=1, multiobjective=(None, 0), const_func=100, fac_ratio=2.1, modes=None, rho=7860, E=210e9, v=0.3, x_min_m=0.001, x_min_k=0.001, alpha_par=0, beta_par=5e-6, eta_par=0, alpha_plot=0, beta_plot=1e-8, eta_plot=0, p_par=3, q_par=1, passive_coord=None, freq_rsp=[], dens_filter=True, each_iter=True, max_iter=100, mesh_deform=False, factor=1000, save=False, timing=False):
    ...
    while (kktnorm > kkttol) and (outit < max_iter) and (kconv < 5):
        ...
        # Constraint function       
        fvalnew, _ = opt.apply_constr(fval, dfdx, constr_func, constr_values, freq_constr, ind_constr2, lx, ly, ngl, coord, connect, E, v, rho, alpha_par, beta_par, eta_par, 
                                             p_par, q_par, x_min_m, x_min_k, area, xnew, modes, disp_vector, dyna_stif, stif_matrix, mass_matrix, damp_matrix, load_vector, omega1_par, const_func, free_ind, passive_el, ind_dofs, gradients=False)
        
        if freq_constr_bool:
            fvalnew = opt.normalize_constr(fvalnew, f_scale_constr)
        
        fvalnew[:, 0] -= constr_values
        ...
        if multiobj_bool:
            ...
            if dens_filter:
                ...
                dfdx, df0dx, _ = opt.sensitivity_filter(H, neighbors, x_min_k, xval, dfdx, df0dx)
            # Normalize objective f
            f0val, df0dx = opt.normalize(n1, f0_scale, f0val, df0dx)   
