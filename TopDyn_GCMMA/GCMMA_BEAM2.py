...
def main(nelx, nely, lx, ly, func_name, force_matrix, restri_matrix=None, freq1=180, constr_func=['Area'], constr_values=[50], n1=1, multiobjective=(None, 0), const_func=100, fac_ratio=2.1, modes=None, rho=7860, E=210e9, v=0.3, x_min_k=1e-8, x_min_m=1e-12, alpha_par=0, beta_par=5e-6, eta_par=0, alpha_plot=0, beta_plot=1e-8, eta_plot=0, p_par=3, q_par=1, freq_rsp=[], dens_filter=True, each_iter=True, max_iter=100, mesh_deform=False, factor=1000, save=False, timing=False):
    ...
    while (kktnorm > kkttol) and (outit < max_iter):
        ...
        if modes is not None:
            ...
            disp_vector, t_harmonic = opt.harmonic_problem(ngl, dyna_stif, load_vector, free_ind)
        # Area function
        fvalnew, _ = opt.apply_constr(fvalnew, dfdx, constr_func, constr_values, nelx, nely, lx, ly, coord, connect, E, v, rho, alpha_par, beta_par, p_par, q_par, x_min_k, x_min_m, area, xnew, disp_vector, dyna_stif, stif_matrix, mass_matrix, omega1_par, const_func, free_ind, gradients=False)
        # Objective function      
        if func_name == "Compliance":
            f0valnew = opt.func_compliance(disp_vector, load_vector)
        #
        elif func_name == "Elastic Potential Energy":
            f0valnew, fvirg = opt.elastic_potential_energy(disp_vector, stif_matrix, const_func)
        #
        elif func_name == "Input Power":
            f0valnew, fvirg = opt.func_input_power(disp_vector, load_vector, omega1_par, const_func)
        #               
        elif func_name == "Kinetic Energy":
            f0valnew, fvirg = opt.kinetic_energy(disp_vector, mass_matrix, omega1_par, const_func)
        #   
        elif func_name == 'R Ratio':
            f0valnew, fvirg, elastic_p, kinetic_e = opt.R_ratio(disp_vector, stif_matrix, mass_matrix, omega1_par, const_func)
        # Normalization
