    ...
    **Bases:** :class:`GraphicsObject <pyqtgraph.GraphicsObject>`
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'cmap' in kwargs.keys():
            ...
            if kwargs['cmap'] in graphicsItems.GradientEditorItem.Gradients.keys():
                self.cmap = kwargs['cmap']
            else:
                raise NameError('Undefined colormap, should be one of the following: '+', '.join(['"'+i+'"' for i in graphicsItems.GradientEditorItem.Gradients.keys()])+'.')
        else:
            self.cmap = 'viridis'
    ...
    def setData(self, *args):
        ...
        pos   = [i[0] for i in graphicsItems.GradientEditorItem.Gradients[self.cmap]['ticks']]
        color = [i[1] for i in graphicsItems.GradientEditorItem.Gradients[self.cmap]['ticks']]
        cmap  = ColorMap(pos, color)
        #lut   = cmap.getLookupTable(0.0, 1.0, 256)
        lut   = cmap.getLookupTable(1.0, 0.0, 256)
